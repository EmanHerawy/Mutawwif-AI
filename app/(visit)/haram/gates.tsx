...
export default function HaramGatesScreen() {
  const { i18n } = useTranslation();
  const isAr = i18n.language.startsWith('ar');

  const haramGates = useGateStore((s) => s.haramGates);
  const haramDataSource = useGateStore((s) => s.haramDataSource);
  const loading = useGateStore((s) => s.loading);
  const fetchHaram = useGateStore((s) => s.fetchHaram);

  const [filter, setFilter] = useState<Filter>('all');

  useEffect(() => {
    if (haramGates.length === 0) {
      fetchHaram();
    }
  }, []);

  const sorted: GateInfo[] = [...haramGates].sort((a, b) => {
      ...
      <View style={styles.container}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <GateCard item={item} dataSource={haramDataSource ?? 'mock'} />
            )}
