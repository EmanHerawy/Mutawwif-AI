...
const CATEGORY_META: { key: Filter; labelAr: string; labelEn: string; emoji: string; icon: string }[] = [
  ...
  { key: 'pilgrim_services',    labelAr: 'خدمات الزوار',      labelEn: 'Visitor Services', emoji: '🧳', icon: 'hands-helping' },
...
export default function NabawiFacilitiesScreen() {
  ...
  const counts = useMemo(() => {
    ...
    for (const meta of CATEGORY_META) {
      if (meta.key === 'accessible') {
        map[meta.key] = NABAWI_FACILITIES.filter((f) => f.accessible).length;
      } else {
        map[meta.key] = NABAWI_FACILITIES.filter((f) => f.type === meta.key).length;
      }
    }
    return map;
  }, []);
  ...
  const displayed = useMemo(() => {
    if (!filter) return [];
    if (filter === 'accessible') return NABAWI_FACILITIES.filter((f) => f.accessible);
    return NABAWI_FACILITIES.filter((f) => f.type === filter);
  }, [filter]);
