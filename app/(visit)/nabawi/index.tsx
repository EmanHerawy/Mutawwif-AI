...
export default function NabawiVisitScreen() {
  ...
  const NAV_CARDS = [
    { titleEn: 'Gates', titleAr: 'البوابات', icon: 'door-open', onPress: () => router.push('/(visit)/nabawi/gates') },
    { titleEn: 'The Rawdah', titleAr: 'الروضة الشريفة', icon: 'star-of-life', onPress: () => router.push('/(visit)/nabawi/rawdah') },
    { titleEn: 'Facilities', titleAr: 'الخدمات', icon: 'map-marked-alt', onPress: () => router.push('/(visit)/nabawi/facilities') },
  ];
  ...
  if (selectedCat) {
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <Stack.Screen options={{ title: isAr ? 'المسجد النبوي' : 'Masjid Al Nabawi' }} />
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerCard}>
          <View style={styles.headerIconBox}>
            <FontAwesome5 name="place-of-worship" size={32} color={Colors.goldAccent} solid />
          </View>
          <Text style={styles.headerTitle}>{isAr ? 'المسجد النبوي' : 'Masjid Al Nabawi'}</Text>
          <Text style={styles.headerSubtitle}>
            {isAr ? 'بوابات • الروضة • خدمات • آداب' : 'Gates • Rawdah • Facilities • Etiquette'}
          </Text>
        </View>
        ...
        <Text style={styles.sectionLabel}>{isAr ? 'آداب المسجد النبوي' : 'Nabawi Etiquette'}</Text>
...
const styles = StyleSheet.create({
  ...
  // Section label
  sectionLabel: {
    fontSize: 11, fontWeight: '700', color: Colors.brandGreen, opacity: 0.6,
    textTransform: 'uppercase', letterSpacing: 0.5,
    marginBottom: 10, marginTop: 4,
  },
  // Nav cards — stacked column layout for 3 cards
  navGrid: { gap: 10, marginBottom: 20 },
  navCard: {
