...
export function buildSOSMessage(payload: SOSPayload): string {
  ...
  return [
    ...
    persona.hotelMakkahName ? `Hotel Makkah: ${persona.hotelMakkahName}` : '',
    persona.hotelMadinahName ? `Hotel Madinah: ${persona.hotelMadinahName}` : '',
    persona.groupGuidePhone ? `Group Guide: ${persona.groupGuideName || ''} ${persona.groupGuidePhone}` : '',
    `Last GPS: ${coordStr}`,
    `Time: ${timeStr}`,
